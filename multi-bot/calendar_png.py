...
def generate_calendar(month_number):
    ...
    ax.set_title(f"{calendar.month_name[month_number]} {year}", fontsize=14, fontweight="bold")

    # Отрисовываем названия дней недели
    days_of_week = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"]
    for i, day in enumerate(days_of_week):
        ax.text(i, 0, day, fontsize=12, ha="center", va="center", fontweight="bold")
    ...
    for week_idx, week in enumerate(cal):
        for day_idx, day in enumerate(week):
            ...
            ax.text(day_idx, -week_idx - 1, str(day), fontsize=20, ha="center", va="center", color=color,
                    fontweight="bold")
